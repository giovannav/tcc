...
def train_model(num_epochs, img_shape, batch_size, learning_rate):
    ...
    csv_logger = CSVLogger(filename=f'results_csv/VGG19-{timestamp_start}.csv', separator=',', append=False)
...
def evaluate_model(model, history, test_data_gen, timestamp_start, num_epochs):
    ...
    model_name = f'VGG19-{timestamp_start}-{timestamp}'
...
def load_model(model_name, num_output_nodes, num_epochs, img_shape, batch_size, learning_rate):
    ...
    vgg_model = VGG16(weights='imagenet', include_top=False, input_shape=(img_shape, img_shape, 3))
